...
#endif
/* Make sure we implicitly promote constants from unsigned int to unsigned LONG64
 ...
 */
LONG64 negative_one = 1l; // can't use negative static initializers; negate this in main
LONG64 zero = 0l;
...
int main(void) {
     ...
     * so it will be promoted to an unsigned LONG64;
     * when we compare this to -1l, we'll convert -1l to
     * an unsigned LONG64 with value ULONG_MAX
     */
     ...
     * is promoted to signed LONG64, not an unsigned int,
    ...
    /* constants with ul suffix are always treated as unsigned LONG64, not unsigned int
